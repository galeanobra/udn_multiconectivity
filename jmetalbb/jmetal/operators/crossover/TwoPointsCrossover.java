...
public class TwoPointsCrossover extends Crossover {
     ...
     */
    private static List VALID_TYPES = Arrays.asList(PermutationSolutionType.class, BinarySolutionType.class);
    ...
    public Solution[] doCrossover(double probability,
        ...
        else if (parent1.getType().getClass() == BinarySolutionType.class) {
            if (PseudoRandom.randDouble() < probability) {
                int crosspoint1;
                int crosspoint2;
                int individualLength;
                Binary parent1Vector;
                Binary parent2Vector;
                Binary offspring1Vector;
                Binary offspring2Vector;

                individualLength = ((Binary) parent1.getDecisionVariables()[0]).getNumberOfBits();
                parent1Vector = (Binary) parent1.getDecisionVariables()[0];
                parent2Vector = (Binary) parent2.getDecisionVariables()[0];
                offspring1Vector = (Binary) offspring[0].getDecisionVariables()[0];
                offspring2Vector = (Binary) offspring[1].getDecisionVariables()[0];

                // STEP 1: Get two cutting points
                crosspoint1 = PseudoRandom.randInt(0, individualLength - 1);
                crosspoint2 = PseudoRandom.randInt(0, individualLength - 1);
                ...
                //STEP 2: Obtain the first child
                for (int k = crosspoint1; k <= crosspoint2; k++) {
                    offspring1Vector.setIth(k, parent2Vector.getIth(k));
                } // for

                // STEP 3: Obtain the second child
                for (int k = crosspoint1; k <= crosspoint2; k++) {
                    offspring2Vector.setIth(k, parent1Vector.getIth(k));
                } // for
            }

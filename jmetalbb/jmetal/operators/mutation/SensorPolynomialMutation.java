 ...
 */
public class SensorPolynomialMutation extends Mutation {
     ...
     */
    public SensorPolynomialMutation(HashMap<String, Object> parameters) {
        super(parameters);
    ...
    public void doMutation(double probability, Solution solution) throws JMException {
        ...
        for (int var = 0; var < solution.getDecisionVariables().length; var++) {
            if (PseudoRandom.randDouble() <= probability) {
                ...
                } else {//the sensor is deployed, so either move or remove it
                    //if (PseudoRandom.randDouble() < 0.5) { //remove
                    if (PseudoRandom.randDouble() < 0.5) { //remove
                        s.setDeployed(false);
                    } else { //move

                        //apply Polynomial Mutation to coordinate X
                        y = ((Sensor) solution.getDecisionVariables()[var]).getX();
                        yl = solution.getDecisionVariables()[var].getLowerBound();
                        yu = ((Sensor) solution.getDecisionVariables()[var]).getUpperBoundX();
                        delta1 = (y - yl) / (yu - yl);
                        delta2 = (yu - y) / (yu - yl);
                        ...
                        ((Sensor) solution.getDecisionVariables()[var]).setX((int) y);

                        //apply Polynomial Mutation to to coordinate y
                        y = ((Sensor) solution.getDecisionVariables()[var]).getY();
                        yl = solution.getDecisionVariables()[var].getLowerBound();
                        yu = ((Sensor) solution.getDecisionVariables()[var]).getUpperBoundY();
                        delta1 = (y - yl) / (yu - yl);
                        delta2 = (yu - y) / (yu - yl);
                        rnd = PseudoRandom.randDouble();
                        mut_pow = 1.0 / (eta_m_ + 1.0);
                        ...
                        ((Sensor) solution.getDecisionVariables()[var]).setY((int) y);
                    }
                }
            }
        }

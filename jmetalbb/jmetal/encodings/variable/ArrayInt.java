...
/**
 * Class implementing a decision variable representing an array of integers. The
 * integer values of the array have their own bounds.
 */
public class ArrayInt extends Variable {
    ...
    public ArrayInt(int size, Problem problem) {
        ...
        for (int i = 0; i < size_; i++) {
            int lb = (int)problem_.getLowerLimit(0);
            int ub = (int)problem_.getUpperLimit(0);
           array_[i] = PseudoRandom.randInt(lb,ub);
           lowerBounds_[i] = lb;
           upperBounds_[i] = ub;
        } // forﬁ
    }

...
/**
 * DNSGA-II with an Adyacent BTS Restart operator
 *
 * @author paco
 */
public class DNSGAII_ABR extends Algorithm {
     ...
     */
    public DNSGAII_ABR(Problem problem) {
        super(problem);
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        abrRate = ((Double) getInputParameter("abrRate")).doubleValue();
        abrLength = ((Integer) getInputParameter("abrLength")).intValue();
        ...
        for (int epoch = 0; epoch <= ((DynamicCSO) problem_).getNumberOfEpochs(); epoch++) {
            ...
            for (int i = 0; i < restartSolutions; i++) {
                ...
                Solution s = new Solution(population.get(permutation[i]));
//                System.out.println("Before: "+ s);
                ((DynamicCSO) problem_).adjacentBtsRestart(s, abrLength, abrRate);
                problem_.evaluate(s);
//                System.out.println("After: " + s + "\n");
                problem_.evaluateConstraints(s);
                evaluations++;

                population.remove(permutation[i]);
                population.add(s);

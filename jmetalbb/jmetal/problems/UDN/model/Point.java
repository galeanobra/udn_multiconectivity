...
public class Point {
     ...
     * @return
     */
    public ArrayList<Cell> getCellsWithHigherSINR(int NCells, User u) {
        double sinr;
        Map<Double, Double> maxSINR = new TreeMap<>();
        Map<Double, Cell> servingBTSs = new TreeMap<>();
        Map<Double, Cell> SortedBTSs = new TreeMap<>();
        ArrayList<Cell> servingCells = new ArrayList<>();
        ArrayList<UDN.CellType> FrequenciesSupported = new ArrayList<>() ;
        FrequenciesSupported = u.getSupportedAntennas();
        double maxValueSINR = Double.NEGATIVE_INFINITY;
        ...
        for (Double frequency : this.udn_.cells_.keySet()) {
            for (Cell c : udn_.cells_.get(frequency)) {
                ...
                if (c.isActive()) {
                    ...
                    for (UDN.CellType antenna: FrequenciesSupported){

                        if (c.getType() == antenna){
                            ...
                            if (sinr > maxValueSINR) {
                                maxValueSINR = sinr; // Cell with higher value of SINR for User u
                            }
                            break;
                        }
                    }
        ...
        //retrieve the best Ncells among the precomputed values and sorting them(Decreasing)
        do{
            double maxValue = Double.NEGATIVE_INFINITY;
            Cell aux_cell = null;
            double aux_freq = 0.0;
            double ValueToDelete = 0.0;

            for (Double f : servingBTSs.keySet()) {
                sinr = maxSINR.get(f);
                if (sinr > maxValue) {
                    maxValue = sinr;
                    aux_cell = servingBTSs.get(f);
                    aux_freq = f;
                    ValueToDelete = f;
                }
            }
            if (servingBTSs.size() > 1){

                servingBTSs.remove(ValueToDelete);

            }
            SortedBTSs.put(aux_freq, aux_cell);
        }while((SortedBTSs.size() < NCells) && (FrequenciesSupported.size() >= NCells));

        //double umbral = maxValueSINR*this.udn_.getMCThreshold();

        /*
           applying Threshold for MC
         */
        for (Double f : SortedBTSs.keySet()) {
            sinr = maxSINR.get(f);
            if (sinr > maxValueSINR*this.udn_.getMCThreshold()) {
                 servingCells.add(SortedBTSs.get(f));
            }
        }
        return servingCells;
    }
